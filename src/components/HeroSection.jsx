...
const HeroSection = () => {
  ...
  const handleFbPage = ({target}) => {
    ...
    return (
      <div className='w-full flex justify-start relative lg:mt-10 xl:mt-0 2xl:justify-stretch' >
        <div className='flex relative mx-auto text-center items-center text-darkBrown hero-section lg:text-left 2xl:mx-0'>
          ...
          <div>
            ...
            <div className='relative mx-auto hero-img lg:absolute lg:right-4 lg:top-1/2 lg:-translate-y-1/2 xl:right-0'>
              <MessageModal 
                ...
                position={"-left-10 top-2  lg:top-5 lg:-left-14 3xl:-left-[8em]"} />

              <MessageModal 
                message={<>Elevate your dining experience with us today!&#128525;</>}
                trianglePos={"bottom-0 left-2"} 
                position={"-right-10 bottom-24 xs:bottom-28 lg:bottom-40  lg:-right-14 xl:bottom-60 xl:-right-10 3xl:-right-16"} />
          ...
          <p className='leading-9 text-sm sm:text-base lg:pr-20 xl:text-lg xl:leading-10 3xl:text-xl'>Enjoy elegant cuisine and qualitty service in an ambiance tailored to create lasting memories.</p>

          <div className='text-xs hero-customers justify-center font-semibold text-lightOrange flex items-center xs:text-sm 3xl:text-lg'>
             <div className='flex happy-customers'>
              <img className='w-5 border border-pureWhite xs:w-8 3xl:w-10 rounded-full' src={jhonwellImg} alt='' />
              <img className='w-5 border border-pureWhite xs:w-8 3xl:w-10 rounded-full -translate-x-1/4' src={kenzoImg} alt='' />
              <img className='w-5 border border-pureWhite xs:w-8 3xl:w-10 rounded-full -translate-x-1/2' src={marwinImg} alt='' />
              <img className='w-5 border border-pureWhite xs:w-8 3xl:w-10 rounded-full -translate-x-3/4' src={kingpaulImg} alt='' />
             </div>

             <p>700+ Happy Customers</p>
          </div>

          <div className='flex gap-7 flex-col items-center hero-btns lg:flex-row lg:mt-3 3xl:mt-6'>
            <button onClick={() => {navigate('/products'); resetScroll()}} className='text-xs bg-lightOrange text-pureWhite p-4 px-9 rounded-full sm:text-sm 3xl:text-base'>EXPLORE MENU</button>
            <button className='relative flex items-center gap-2 text-sm 3xl:text-base'>
...
const MessageModal = ({position, trianglePos, message}) => {
  ...
  return (
    <div className={`absolute ${position} text-darkBrown scale-75 z-20 p-2 bg-pureWhite border-lightOrange w-1/2 max-w-52 border rounded-xl message-modal xxs:p-3 lg:p-4 lg:scale-100 xl:p-5 3xl:max-w-full`}>
