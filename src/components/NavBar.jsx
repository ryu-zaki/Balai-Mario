...
const NavBar = () => {
    ...
    return (
        <>
        {sideNavVisible && <div onClick={() => setSideNavVisible(false)} className="fixed z-10 inset-0 bg-darkOverlay w-full h-full"></div>}

        <div className="flex w-full items-center justify-between py-5 md:py-6 xl:py-8" >
            <h3 className="text-2xl title-font">Balai Mario</h3>

            <nav>
            <ul className="hidden text-sm gap-6 md:flex xl:mr-64 xl:text-base xl:gap-14">
                <li><a href="/">home</a></li>
            ...
            <div className="hidden gap-6 text-xs md:flex xl:relative xl:z-10 xl:text-sm">
                <button className="border-2 border-darkBrown text-darkBrown px-7 p-2 rounded-full xl:border-pureWhite xl:text-pureWhite">Register</button>
                <button className="bg-lightOrange text-pureWhite px-8 p-2 rounded-full xl:bg-pureWhite xl:text-darkBrown">Login</button>
            </div>
            ...
            {/* Side Navigation */}
            <div className={`fixed top-0 side-nav transition-all duration-700  z-10  overflow-y-auto bottom-0 bg-pureWhite left-0 p-5 px-8 flex flex-col gap-10 ${!sideNavVisible && "-translate-x-full"}`} >
